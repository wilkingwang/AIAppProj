...
class DataConversionTool:
    def __init__(self):
        self.name = "数据转换"
        self.descirption = "在不同数据格式之间转换，如Json、CSV等"
...
tools = {
    ...
    "文本处理": RunnableLambda(lambda x: text_processing.run(x["operation"], x["content"]))
}
...
if __name__ == "__main__":
    result = lcel_task_chain("文本分析", {"text": "这个产品非常好用，我很喜欢它的设计，使用体验非常棒！"})
    print("示例1结果：",result)
    print("-" * 30)

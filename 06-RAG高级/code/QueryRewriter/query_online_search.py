...
class WebSearchQueryRewriter:
    ...
    def identify_web_search_needs(self, query, conversation_history=""):
        ...
        """
        instruction = """
你是一个智能查询分析专家。请分析用户查询，判断是否需要联网搜索来获取最新、最准确的信息。
...
"""
    ...
    def auto_web_search_rewrite(self, query, conversation_history=""):
        ...
        # 第二部：改写查询
        rewriter_result = self.rewrite_for_web_search(query, conversation_history)
        ...
        return {
            ...
            "original_query": query,
            "rewritten_query": rewriter_result.get('rewritten_query', query),
            "search_keywords": rewriter_result.get('search_keywords', []),
            "search_intent": rewriter_result.get('search_intent', ''),
            "suggested_sources": rewriter_result.get('suggested_sources', []),

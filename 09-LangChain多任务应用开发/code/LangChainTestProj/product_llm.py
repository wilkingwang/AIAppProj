...
现在开始回答，记得在给出最终答案前，需要按指定格式进行一步一步的推理。
Question: {input}
{agent_scratchpad}
"""
...
class CustomPromptTemplate(StringPromptTemplate):
    template: str   # 标准模板
    tools: List[Tool]   # 开使用工具集合

    def format(self, **kwargs) -> str:
        """
        按照定义的 template，将需要的值填写进去
        Returns:
            str： 填写好后的template
        """
...
class CustomOutputParser(AgentOutputParser):
    def parse(self, llm_output: str) -> Union[AgentAction, AgentFinish]:
        """
        解析llm的输出，根据输出文本找到需要执行的决策
        """
        #如果句子中包含 Final Answer则代表已经完成
        ...
        action_intput = match.group(2)

        # Agent执行
        return AgentAction(
            tool=action, tool_input=action_intput.strip(" ").strip(" "), log=llm_output
        )

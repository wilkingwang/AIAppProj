...
基于LangGraph实现的混合型智能体，结合反应式架构的即使响应能力和深思熟虑架构的长期规划能力，通过协调层动态切换处理模型

三层架构：
1. 底层（反应式）：即使响应客户查询，提供快速反馈
...
from typing import Any, Literal, List, Dict, Optional, TypedDict
...
class CustomerProfile(BaseModel):
    ...
    客户画像信息
    """
    custom_id: str = Field(..., description="客户ID")
...
class InvestmentAnalysisOutput(BaseModel):
    ...
    portfolio_analysis: Dict[str, Any] = Field(..., description="组合分析")
    recommendation: List[Dict[str, Any]] = Field(..., description="投资建议")
...
class WealthAdvisorState(TypedDict):
    ...
    # 输入
    user_query: str # 用户查询
    customer_profile: Optional[Dict[str, Any]] = None # 客户画像
    ...
    emergency_response: Optional[Dict[str, Any]] = None # 紧急响应结果
...
def assess_query(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        print("1. 情境评估，LLM评估输出：", result)
        print(f"1. 情境评估，分支判断：processing_mode={result.get('processing_mode', '未知')}， query_type={result.get('query_type', '未知')}")
        ...
        print(f"1. 情境评估，更新状态：{updated_state}")
        ...
        return {
            **state,
            ...
            "final_response": f"评估查询时发生错误，无法处理您的请求",
        }
...
def reactive_processing(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        print(f"反应式处理阶段出错：{str(e)}")
        return {
            **state,
            "error": f"反应式处理阶段出错：{str(e)}",
            "final_response": f"反应式处理查询时发生错误，无法处理您的请求",
        }
...
def collect_data(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        # 准备提示
        prompt = ChatPromptTemplate.from_template(COLLECT_DATA_PROMPT)

        # 构建输入
        input_data = {
            "user_query": state["user_query"],
            "customer_profile": json.dumps(state.customer_profile, ensure_ascii=False, indent=2),
        }
...
def generate_recommendations(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        return {
            **state,
            "error": f"建议阶段出错：{str(e)}",
            "current_phase": "recommend",
        }

# 创建智能体工作流
def create_wealth_advisor_agent() -> StateGraph:
    """
    创建财富顾问混合智能体工作流
    """
    ...
    # 添加节点
    graph.add_node("assess", assess_query)
    graph.add_node("reactive", reactive_processing)
    ...
    # 定义一个显式的响应节点函数
    def response_function(state: WealthAdvisorState) -> WealthAdvisorState:
        """
        最终响应生成节点，原样返回状态
        """
        print(f"response_function：{state}")
    ...
    graph.add_node("respond", response_function)
    ...
    # 添加固定路径边
    graph.add_edge("reactive", "respond")
...
# 运行智能体
def run_wealth_advisor(user_query: str, customer_id: str = 'customer1') -> WealthAdvisorState:
    """
    运行财富顾问智能体
    """

    # 创建工作流
    agent = create_wealth_advisor_agent()
